...
public class FunctionElementNodeEditPolicy extends GraphicalNodeEditPolicy {
	...
	protected Command getConnectionCompleteCommand(
		...
		ConnectionAnchor ctor = getFEEditPart().getTargetConnectionAnchor(request);
		if (ctor == null) {
			return null;
		}
	...
	protected Command getConnectionCreateCommand(CreateConnectionRequest request) {
		...
		command.setStartAnchor(getFE().getFunction().getId().toString());
	...
	protected Command getReconnectSourceCommand(ReconnectRequest request) {
		System.out.println("getReconnectSource"); //$NON-NLS-1$
		return null;
	}

	@Override
	protected Command getReconnectTargetCommand(ReconnectRequest request) {
		System.out.println("getReconnectTarget"); //$NON-NLS-1$
		return null;
	}
	...
	private FunctionElementEditPart getFEEditPart() {
		return (FunctionElementEditPart) getHost();
	}

	private ConnectedFigure getConnectedFigure() {
		return (ConnectedFigure) getFEEditPart().getFigure();
	}
	
	private FunctionElement getFE() {
		return (FunctionElement) getFEEditPart().getModel();
	}
