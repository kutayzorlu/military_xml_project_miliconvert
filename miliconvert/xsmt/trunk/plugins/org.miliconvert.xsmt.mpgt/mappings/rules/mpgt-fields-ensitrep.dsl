...
[consequence][]VI {mpgt} "HO" {tmp} : LIGNE {ligne}=EnsitrepFieldRules.VI_ETAT_OP($s,$f,"{mpgt}","{ligne}");
[consequence][]VD {mpgt} "HO" {tmp} : LIGNE {ligne}=EnsitrepFieldRules.VD_ETAT_OP($s,$f,"{mpgt}","{ligne}");
[consequence][]VI {mpgt} \= nom {tmp} ligne {ligne}=EnsitrepFieldRules.VI_NOM_ENTITE($s,$f,"{mpgt}","{ligne}");
[consequence][]CD A_LOCATION_TYPE pour traiter la conversion des valeurs {conv}=EnsitrepFieldRules.CD_A_LOCATION_TYPE($s,$f,"{conv}");
[consequence][]Règle {rule_name}=EnsitrepFieldRules.{rule_name}($s,$f);
[consequence][]Concaténation des {count} sous zones dans l'attribut {value}=EnsitrepFieldRules.concatSubFields($s,$f,"{count}","{value}") ;
...
[consequence][]VC {field}\={value}=if(!$s.isSkip())CommonFieldRules.VC($s,"{field}",{value});
