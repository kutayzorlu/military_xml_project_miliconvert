...
public class FunctionExtensionParser {
	...
	public FunctionExtensionParser() {
		...
		for (IExtension extension : extensions) {
			...
			for (IConfigurationElement element : extension
				...
				System.out.println("configElement.name: " + element.getName()); //$NON-NLS-1$
				System.out.println("Adding plug-in provided function..."); //$NON-NLS-1$
				if (element.getName().equals("userFunction")) {
					functionExtensionParsed.add(parse(new FunctionExtension(
							element), element));
				}
			}
		}
	...
	private FunctionExtension parse(FunctionExtension functionExtension,
		...
		functionExtension.setFunctionCategory(FunctionCategory
				.create(configurationElement.getAttribute(FUNCTION_CATEGORY)));
