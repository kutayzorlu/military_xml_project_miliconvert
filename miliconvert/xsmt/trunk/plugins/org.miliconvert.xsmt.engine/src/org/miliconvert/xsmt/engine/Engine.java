...
public class Engine implements ITModelListener {
	...
	public void run(Result r) throws EngineException {
		Activator.getDefault().logInfo("engine execution starting");
		...
		try {
			...
			Activator.getDefault().logInfo("engine execution successfull");
	...
	public void unregisterListener(IEngineListener el) {
		listeners.remove(el);
	}
