...
public class XSMTEditor extends GraphicalEditorWithFlyoutPalette {
	...
	protected void setInput(IEditorInput input) {
		...
		try {
			...
			if (runEngineAction != null) {
				runEngineAction.setEngine(diagram.getEngine());
			}
			if (setXsiAction != null) {
				setXsiAction.setEngine(diagram.getEngine());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
				...
				new ProgressMonitorDialog(shell).run(false, // don't fork
						...
						new WorkspaceModifyOperation() { // run this
							...
							public void execute(final IProgressMonitor monitor) {
								try {
									if (file.exists()) {
										file.setContents(diagram.save(), true,
												false, monitor);
									} else {
										file.create(diagram.save(), true,
												monitor);
									}
								} catch (Exception ce) {
									ce.printStackTrace();
