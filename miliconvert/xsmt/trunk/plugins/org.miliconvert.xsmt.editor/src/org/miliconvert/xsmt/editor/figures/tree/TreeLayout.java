...
public class TreeLayout extends AbstractLayout {
	...
	public void layout(IFigure container) {
		...
		for (int i = 0; i < subtrees.size(); i++) {
			...
			subtree.setLocation(transposer.t(currentXY.getTranslated(-overlap,
					0)));

			// Setup value for next sibling
			int advance = gap + subtreeSize.width - overlap;
			rightContour = calculateNewRightContour(rightContour, subtree
					.getContourRight(), advance);
			currentXY.x += advance;
			...
			if (shiftRight > 0) {
				...
				for (int j = 0; j <= i; j++)
					((IFigure) subtrees.get(j)).translate(correction.x,
							correction.y);
			}
			...
			if (contactDepth > previousSubtreeDepth) {
				...
				int slack = transposer.t(subtree.getBounds()).x
						- transposer.t(branch.getBounds()).right()
						- gap
						+ calculateOverlap(branch.getContourRight(), subtree
								.getContourLeft());
				int end = i;
				int begin = end - 1;
				while (begin > 0
				...
				for (int j = begin + 1; j < end; j++) {
					...
					Point shift = transposer.t(new Point(slack * (j - begin)
							/ (end - begin), 0));
