...
class NormalLayout extends BranchLayout {
	...
	public void layout(IFigure f) {
		...
		Rectangle nodeLocation = new Rectangle(topLeft, transposer.t(node
				.getPreferredSize()));
		...
		if (!contents.isVisible() || contents.getChildren().isEmpty()) {
			...
			contents.setBounds(transposer.t(nodeLocation.getTranslated(0,
					rowHeight).setSize(0, 0)));
			return;
		}

		Rectangle contentsLocation = new Rectangle(topLeft, transposer
		...
		TreeBranch lastChild = (TreeBranch) contents.getChildren().get(
				contents.getChildren().size() - 1);
		int leftInset = firstChild.getContourLeft()[0]
				+ transposer.t(firstChild.getBounds()).x
				- transposer.t(contents.getBounds()).x;
		int rightInset = lastChild.getContourRight()[0]
		...
		int adjust = branchBounds.width
				- Rectangle.SINGLETON.setBounds(contentsLocation).union(
						nodeLocation).width;
		adjust /= 2;
	...
	void mergeContour(int[] destination, int[] source, int startdepth,
			int offset) {
	...
	void paintLines(Graphics g) {
		if (getTransposer().isEnabled()) {
			...
			for (int i = 0; i < children.size(); i++) {
				int y = ((TreeBranch) children.get(i)).getNodeBounds()
						.getCenter().y;
			...
			int xMax = xMid;
			for (int i = 0; i < children.size(); i++) {
				int x = ((TreeBranch) children.get(i)).getNodeBounds()
						.getCenter().x;
	...
	void updateContours() {
		...
		Rectangle clientArea = transposer.t(branch.getNodeBounds().getUnion(
				branch.contents.getBounds()));
		Rectangle nodeBounds = transposer.t(branch.getNodeBounds());
		transposer.t(branch.getContentsPane()
				.getBounds());
		...
		for (int i = 0; i < subtrees.size() && currentDepth < getDepth(); i++) {
			...
			if (subtree.getDepth() > currentDepth) {
				...
				int leftOffset = transposer.t(subtree.getBounds()).x
						- clientArea.x;
				mergeContour(cachedContourLeft, leftContour, currentDepth,
						leftOffset);
		...
		for (int i = subtrees.size() - 1; i >= 0 && currentDepth < getDepth(); i--) {
			...
			if (subtree.getDepth() > currentDepth) {
				...
				int rightOffset = clientArea.right()
						- transposer.t(subtree.getBounds()).right();
				mergeContour(cachedContourRight, rightContour, currentDepth,
						rightOffset);
				currentDepth = subtree.getDepth();
			}
		}
	...
	void updateRowHeights() {
		...
		for (int i = 0; i < subtrees.size(); i++) {
			...
			for (int row = 0; row < rowHeights.length; row++)
				preferredRowHeights[row + 1] = Math.max(
						preferredRowHeights[row + 1], rowHeights[row]);
		}
	}

	/**
	 * @see org.eclipse.draw2d.examples.tree.BranchLayout#setRowHeights(int[],
	 *      int)
	 */
	@SuppressWarnings("unchecked") //$NON-NLS-1$
