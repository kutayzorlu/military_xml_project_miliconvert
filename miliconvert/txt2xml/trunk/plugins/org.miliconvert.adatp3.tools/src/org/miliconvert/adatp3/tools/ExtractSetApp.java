...
public class ExtractSetApp implements IApplication {
	...
	public Object start(IApplicationContext context) throws Exception {
		...
		try {
		    ...
		    if(cmd.hasOption( "h" )){
		    	formatter.printHelp("extractsetapp", options );
		    	return IApplication.EXIT_OK;
		    }

		    if( ! cmd.hasOption( "f" ) || !cmd.hasOption("n")) {
				formatter.printHelp("extractsetapp", options, true );
				return IApplication.EXIT_OK;
		    }
			...
			if (html.isDirectory()) {
				...
				for (File csv : csvs) {
					extractCSVFromTable(tableToExtract,
							outputDir,BaseLineFactory.getSetExtractor(cmd.getOptionValue( "bl" ), new Openable(csv.getAbsolutePath())));
				}
			} else {
				extractCSVFromTable( tableToExtract, outputDir,BaseLineFactory.getSetExtractor(cmd.getOptionValue( "bl" ), new Openable(givenFileName)));
			}
